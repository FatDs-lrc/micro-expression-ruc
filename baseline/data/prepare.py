...
def clean_not_found(json_file):
    ans = []
    data = json.load(open(json_file))
    print('previous length: {}'.format(len(data)))
    for d in data:
        path = d['path']
        path = os.path.join(img_root, path)
        if os.path.exists(path):
            ans.append(d)
    print('cleaned length: {}'.format(len(ans)))
    json.dump(ans, open(json_file, 'w'), indent=4)
...
# clean_not_found('auxiliary/single_img_test.json')
# clean_not_found('auxiliary/single_img_train.json')
